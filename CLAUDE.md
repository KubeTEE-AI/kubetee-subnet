...
- NIM Operator (experimental Kata & Dynamo support):
  ...
  - **Kata runtime-rs vCPU gotchas (KubeTEE, verifiable — DeepSeek-V4-Flash CC deployment, 2026-07):** the `kata-qemu-nvidia-gpu-tdx-runtime-rs` RuntimeClass boots CC guests with `default_vcpus=1` (config) — a severe bottleneck for CPU-bound guest workloads (LLM weight load, DeepGEMM JIT, CUDA-graph capture) despite ample host cores. Two upstream runtime-rs bugs (both fixed locally by a KubeTEE-patched shim): (1) the `io.katacontainers.config.hypervisor.default_vcpus` pod annotation is **ignored** — qemu boots `-smp 1` regardless ([kata-containers#13439](https://github.com/kata-containers/kata-containers/issues/13439)); (2) `resources.limits.cpu` triggers vCPU **hotplug** (`-smp 65`), but the runtime-rs hotplug path is **unstable** — the container never starts, qemu exits after ~20min, and the shim gets stuck in a `Task/State got error timed out` loop ([kata-containers#13440](https://github.com/kata-containers/kata-containers/issues/13440)). **KubeTEE fix:** a patched shim (`v4.0.0-nvswitch-vcpu-fix2`, deployed via the `kata-deploy-shim-overlay` Fleet DaemonSet) honors the `default_vcpus` annotation by re-applying it **after** `setup_config` (which otherwise resets `default_vcpus` to 1 when a pod has a memory limit but no CPU limit — sidestepping the unstable #13440 hotplug path entirely). KubeTEE CC inference pods set `io.katacontainers.config.hypervisor.default_vcpus: "16"` (no `limits.cpu`) → qemu boots `-smp 16` directly. **CONFIRMED WORKING on TDX** (2026-07-23): the `-smp 16` guest boots (TDX attestation → kata agent connects → all 8 GPU CDI devices injected → kata-direct volume hotplugged → guest-pulls the image) and uses **multiple vCPUs** (qemu ~476% CPU = 4.76 cores during the nydus guest image-pull) — far faster for CPU-bound work (weight load, DeepGEMM JIT, CUDA-graph capture) than `-smp 1` (~30-45min JIT on 1 vCPU). NOTE: an earlier "TDX guest panics with >1 vCPU" conclusion was a **misdiagnosis** — the first `-smp 16` attempt's qemu exit (~1min) was a sandbox-create timeout / premature shim kill, NOT a guest panic; the retry proved `-smp 16` boots fine. `-smp 64` was not retested (its earlier "hang" may also have been premature). See `../nim/deepseek-v4-flash-sglang-fp8-cc.yaml`, `../nim/deepseek-v4-flash-dspark-sglang-cc.yaml`.
  - **Kata runtime-rs teardown lesson (KubeTEE):** the shim's `Task/State got error timed out` journal loop is the **stuck** signal. If the journal shows **0** timeout loops, the sandbox teardown is just **slow** (graceful, ~3-5min) — **do not** prematurely `kill -9` the shim; wait. Premature shim kill mid-teardown closes the ttrpc → `StartError` → an orphaned qemu (no shim) holding GPUs via VFIO (risky — can wedge VFIO/iommufd, requiring a node reboot). Killing a stuck shim reaps an already-dead qemu and is safe **only after** the timeout loop is confirmed — distinct from the `kubectl delete pod --force` VFIO/iommufd wedge warning in the root `CLAUDE.md`. Related: [kata-containers#13430](https://github.com/kata-containers/kata-containers/issues/13430) (NVSwitch IOMMU_IGNORE filter), [#11912](https://github.com/kata-containers/kata-containers/issues/11912) (Go shim bridge-ignore).
