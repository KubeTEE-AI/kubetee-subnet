...
- NIM Operator (experimental Kata & Dynamo support):
  ...
  - **Kata runtime-rs vCPU + teardown gotchas** (KubeTEE, verifiable — DeepSeek-V4-Flash CC deployment, 2026-07) are **infrastructure**, documented in `fleet-gitops/infrastructure/CLAUDE.md` (`kata-deploy/` section: patched shim `v4.0.0-nvswitch-vcpu-fix2` honors `default_vcpus`, `-smp 16` confirmed working on TDX) and the root `CLAUDE.md` "Runtime Classes" note. Kept out of this Bittensor-subnet doc.
