...
- **Compensation pricing (weight-bearing, live).** Each cycle a Taostats feed fetches TAO/USD (the chain doesn't know USD) and the on-chain metagraph supplies alpha→TAO (zero delay, `mg.price`); the miner share of emissions is USD-denominated: an `EARNING` miner's score is `usd_target_per_hour × tenure × window_hours ÷ usd_per_alpha`, where `usd_target_per_hour` applies the GPU $/GPU/hour card to that miner's capacity. Whatever miners do not earn is set as weight on the owner UID — the protocol's recycle/burn sink. Because SN90 runs `recycle_or_burn=recycle`, that alpha returns to **unissued supply and never credits the owner's key** ([Bittensor subnet guide](https://www.bittensor.com/docs/guides/subnets): miner emission directed at an owner hotkey is burned or recycled, never paid). The feed is a **hard dependency**: if it fails, the cycle is skipped, the previous on-chain weights persist, and reliability counters freeze — the validator never guesses a price.
...
- The miner/owner split is **dynamic, not a fixed share**: each cycle the validator converts a USD compensation target into Alpha at the live token price. Everything miners do not earn is set as weight on the owner UID, which under `recycle_or_burn=recycle` is **equivalent to a burn from the owner's perspective** — the alpha returns to unissued supply, and no reward lands on the owner key. The owner's only emission is the protocol-fixed **18% owner cut**; weight on the owner UID pays nothing ([Bittensor subnet guide](https://www.bittensor.com/docs/guides/subnets)).
...
| `GET /v1/attestation?nonce=…` + `X-KubeTEE-Nonce` header | Client-facing attestation — fresh TDX quote with the caller's nonce bound into `REPORTDATA` (standalone or inline on chat responses) | Proof that responses come from the confidential gateway guest; verify via Intel Trust Authority or local DCAP ([Client-Facing Attestation](./docs/CLIENT-FACING-ATTESTATION.md)) |
...
[sayGM (SN28)](https://saygm.com/) is an **inference provider** connected to the KubeTEE LiteLLM gateway. The same OpenRouter-style `model` names (`z-ai/glm-5.2`, `z-ai/glm-5.3`, `z-ai/glm-5.3-flash`, `ornith/ornith-1.5-397b`, …) are the models on `llm.kubetee.ai`. Inbound sayGM buyers already hit those SKUs through the KubeTEE miner ([SN28-SAYGM.md](./docs/SN28-SAYGM.md)).
...
Current buyer models: `z-ai/glm-5.2` (`glm-5.2`), `z-ai/glm-5.3`, `z-ai/glm-5.3-flash`, and `ornith/ornith-1.5-397b` (first worldwide availability, 2026-08-20) — [SN28-SAYGM.md](./docs/SN28-SAYGM.md).
...
- [x] **SN28 (sayGM) idle-capacity channel + LiteLLM inference provider** — live 2026-08-19 ([SN28-SAYGM.md](./docs/SN28-SAYGM.md)). SN90 is **not** an inference subnet; Factory AI services keep priority; SN28 gets spare headroom. Same `model` names on `llm.kubetee.ai`. Buyer offers: `z-ai/glm-5.2`, `z-ai/glm-5.3`, `z-ai/glm-5.3-flash`, `ornith/ornith-1.5-397b` — [SN28-SAYGM.md](./docs/SN28-SAYGM.md). **Ornith-1.5-397B** — first worldwide availability, in collaboration with SN28 sayGM (2026-08-20). Free window closed at 14,812,329,857 tokens.
- [x] **Remote miner-cluster backends on the gateway** — production miner clusters (not just the staging cluster) serve Factory SKUs in TEE behind attested mTLS: image generation (2026-09-19) and video+audio generation (2026-09-21) both live end-to-end through `llm.kubetee.ai`, weights and prompts confined to attested guests. Spec + live-verification record: [EAST-WEST-ATTESTED-MTLS.md](./docs/EAST-WEST-ATTESTED-MTLS.md).
...
- [ ] **Confidential model catalogue** — more TEE-served models on `llm.kubetee.ai` (and optionally extra SN28 SKUs). SN28 is a demand channel, **not** the exclusive path to public inference. Every model published in all three [serving configurations](#serving-configurations--every-job-requires-fast-inference) and billed at below-OpenRouter prices per token on the gateway (a demand channel, **not** an SN90 discounted reseller tier — see [Payment methods](#payment-methods)):
  ...
  - [x] **MiniMax-H3** — H200 nodes, joint video+audio generation (FL2VA: text-to-video-and-audio, first/last-frame conditioning) served in TEE through the gateway's OpenAI-compatible `/v1/videos` (create → poll → content), live 2026-09-21. The MiniMax H3 Community License excludes the US/EU/UK/KR; KubeTEE serves it under **written US authorization from MiniMax** (see [SN28-SAYGM.md](./docs/SN28-SAYGM.md) — models & licenses). Ref2VA (reference-conditioned / V2V) partition is a candidate second deployment.
  - [ ] **SOTA embedding model**
  - [ ] **Rerank** — retrieval reranking models on the gateway `/rerank` endpoint
  - [x] **Images** — **FLUX.2-klein-4B** (Apache-2.0) on the gateway `/v1/images/generations`, live 2026-09-19 (b64_json delivery through LiteLLM). Cosmos3-Super additionally available via the `/cosmos3` native passthrough.
  - [x] **Videos** — **MiniMax-H3** on the gateway `/v1/videos` (see the MiniMax-H3 row above), 5–15 s clips at 768p with native stereo audio, fully in-TEE. **Audio+video joint generation is the capability; the Cosmos3-Super SGLang path was evaluated and retired the same day** (no sound head, OOM on long prompts) — its NIM passthrough remains for native API consumers.
...
- [Client-Facing Attestation](./docs/CLIENT-FACING-ATTESTATION.md) — the `llm.kubetee.ai` attestation API for callers: nonce-bound TDX quotes (standalone endpoint + inline chat headers) and verification via Intel Trust Authority or local DCAP
- [East-West Attested mTLS](./docs/EAST-WEST-ATTESTED-MTLS.md) — the live spec for Trustee-issued gateway↔backend TLS
