...
- The **Armada Server and Executors** shown here are themselves scheduled into Kata + CoCo TEE pods in [Phase 1](../README.md#phase-1--expansion), with attestation-gated TLS on the Server-to-Executor control channel. Phase 0 stands Armada up as ordinary workloads; the *task pods* are confidential from the start either way.
...
- Airflow Connections / Variables are backed by manual Kubernetes Secrets applied out-of-band (Vault and the Vault Secrets Operator were removed from the fleet on 2026-07-23); the Armada endpoint and queue names are configured as Airflow connections.
