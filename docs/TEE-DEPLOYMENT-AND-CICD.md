...
A cold-start failure class that is invisible to the workload and to the host journal: the
**firmware itself** can be the bottleneck. The `OVMF.inteltdx.fd` shipped in kata-deploy v4.0.0
performs **eager memory acceptance** for Intel TDX guests — it spends all its time in
`TDCALL [TDG.MEM.PAGE.ACCEPT]` before the guest kernel starts, so a large-memory VM (≥64 GB)
hangs at boot and exceeds the 1200s `create_container` timeout without producing a single
byte of serial output. A 2.5 TB Kimi-K3 inference pod (8× B300 passthrough) hit this on
`am-b300-60`: the sandbox never reached `BdsDxe`, while the same node's distro
`ovmf-inteltdx` (Ubuntu, with lazy-accept enabled by `PcdLazyAcceptPartialMemorySize=512`)
booted a 512 GB TDX VM in under 15 seconds. The root cause is a Config-A OVMF build with
lazy-accept disabled where a Config-B (`IntelTdxX64.dsc`) build is required. KubeTEE filed
the issue with full evidence, root-cause analysis, and three proposed solutions —
[kata-containers#13535][k13535] — and carries a fork branch (`ovmf-tdx-bump-202605`) with a
pre-built Config-B OVMF from `edk2-stable202605` and a reproducible build script until the
fix lands upstream. This is the model for section [1.9](#19-the-stack-is-version-coupled):
the firmware is part of the coupled set, and a firmware that silently hangs large-memory
guests is a cold-start failure that no amount of probe tuning on top will fix.
...
| Issue | Subject |
|-------|---------|
| [kata-containers#13535][k13535] | TDX OVMF eagerly accepts all guest memory — large-memory CC pods hang at boot |
...
[k13535]: https://github.com/kata-containers/kata-containers/issues/13535
