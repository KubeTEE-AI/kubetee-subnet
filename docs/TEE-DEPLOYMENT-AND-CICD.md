...
## Runtime classes (Kata 4.2.0)

Production Kata is **4.2.0** (`kata-deploy` Fleet bundle, vendored chart `4.2.0+kubetee.1`, `updateStrategy: RollingUpdate`, `maxUnavailable: 1`; rolled 2026-09-15). KubeTEE uses **only** the runtime-rs classes — the Go shims (`kata-qemu-nvidia-gpu-tdx`, `kata-qemu-tdx`) are retired.
...
**Still overlayed on 4.2.0** (not in the tag): shim `v4.2.0-fix11649` (MINIMAL — kata-containers[#11649](https://github.com/kata-containers/kata-containers/issues/11649) idempotent `blockdev-add` only; every other fix class proved moot on 4.2.0 in the 2026-09-17 A/B — see `nim/kata-shim-ab/README.md`), OVMF 202608 lazy-accept, CSI `-kubetee3`. **Retired overlays:** `kata-deploy-gpu-extension-overlay` (2026-08-24, `#13471` stock), `nydus-systemd-fix` (2026-08-24, `#13482` stock), `kata-deploy-base-overlay` (2026-09-15, NVRC v0.1.7 stock in 4.2.0).
