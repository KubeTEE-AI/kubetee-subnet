...
| SN97 | [Albedo](https://github.com/unarbos/distil) (Distil) | Competitive model distillation — miners distill a large teacher model (currently `moonshotai/Kimi-K2.6`, ~1T total / ~32B active MoE) into smaller student models (≤33B total params, Kimi-family architecture), scored by a multi-axis composite including KL divergence against the teacher. King-of-the-hill: a challenger earns by beating the reigning champion's aggregate score, then holds the throne until dethroned. Publishes open distilled checkpoints + public duel traces anyone can use for supervised fine-tuning. **Active KubeTEE PoC**: king-of-the-hill GPU evaluations run as one-off Armada jobs on the KubeTEE staging cluster — see `albedo/kubetee/` in the KubeTEE monorepo (Dockerfile, pod templates, Armada job templates). | NeMo Customizer (coding agents / distillation) + eval data | Distillation + duels run in TEE; open distilled checkpoints are reusable as confidential job templates |
