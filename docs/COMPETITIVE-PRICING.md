...
> (fail-soft: live → cache → card). It also fetches TAO/USD + alpha→TAO
> from the Taostats API each cycle to size the miner share in USD; a Taostats
...
> price-feed module fetches TAO/USD and alpha→TAO from Taostats each cycle so
> scoring can size the miner share in USD; a feed failure skips the cycle.
...
- **Phase 0 (to build):** the USD price card plus a Taostats TAO/USD +
  alpha→TAO feed (compensation price feed) that denominates the miner emission
  share in USD each cycle. This is the *compensation* half — it keeps SN90
  miner pay in the Targon band.
- **Phase 0 (to build — first):** the **live Targon supply-side read**
