...
- **Phase 2 billing gets a native Base settlement asset.** The Early Access path is Alpha / Finney TAO; demand-side paid jobs at a resources price per hour are a [Phase 2 roadmap item](README.md#phase-2--paid-jobs). Phase 2 layers **Alpha / TAO paid jobs** and **USDC-on-BASE and TAO-on-BASE** pull-based billing on top. The USDC→TAO hop is now a Base DEX swap (Aerodrome), not a custom KubeTEE bridge. Recycle still happens on Finney: USDC → TAO-on-BASE → CCIP → Finney TAO → SN90 Alpha → spend → recycle.
