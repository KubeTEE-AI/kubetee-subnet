...
_GPU_ORDER = ["B300", "B200", "H200", "H100", "RTX6000"]
...
def build_html(snapshot: dict) -> str:
    ...
    pub_str = datetime.utcfromtimestamp(published_at).strftime(
        "%Y-%m-%d %H:%M:%S UTC"
    )
    ...
    for klass in _GPU_ORDER:
        if klass in targon_payout:
            ...
            arrow = (
                "down"
                if price < committed
                else ("up" if price > committed else "=")
            )
    ...
    chain_emissions = snapshot.get("chain_emissions", {})
    rows = ""
    for m in miners:
        uid = m.get("uid", "?")
        ...
        status = (
            '<span style="color:#3ee98f">validated</span>'
            if ready
            else '<span style="color:#ff8494">not ready</span>'
        )
        gpus = m.get("gpus", [])
        gpu_str = (
            ", ".join(
                f"{html.escape(str(k))}:{html.escape(str(v))}" for k, v in gpus
            )
            if gpus
            else "—"
        )
        ...
        # Alpha/epoch from our scoring
        alpha_epoch = float(m.get("alpha", 0.0))
        # Chain emission (actual alpha paid by Yuma Consensus last epoch)
        chain_em = float(
            chain_emissions.get(str(uid), chain_emissions.get(uid, 0.0)) or 0.0
        )
        share = float(m.get("share", 0.0))
        # Epoch total = payout/hr * hours_per_epoch
        epoch_total = payout_hr * hours_per_epoch
        rows += f"<tr><td>{uid}</td><td>{cluster}</td><td>{status}</td><td>{gpu_str}</td><td class='num'>${_fmt(payout_hr)}/hr</td><td class='num'>${_fmt(usd_hr)}/hr</td><td class='num' style='color:#4f9cff'>{_fmt(chain_em,4)}</td><td class='num'>${_fmt(epoch_total)}</td><td class='num'>{_fmt(share,1)}%</td></tr>\n"
    if not rows:
...
<div class="panel full">
  ...
  <table>
    <thead><tr><th>UID</th><th>Cluster</th><th>Status</th><th>GPUs</th><th class="num">Target rate</th><th class="num">Earned rate</th><th class="num">Chain alpha/ep</th><th class="num">Epoch payout</th><th class="num">% of pool</th></tr></thead>
    <tbody>
{rows}    </tbody>
  </table>
</div>
