...
def test_tenure_scales_earnings():
    ...
    scores = compute_weights(
        ...
        window_hours=1.0,
        owner_uid=0,
        emission_pool_usd=28.0,
    )
    assert scores.weights[1] == pytest.approx(0.5)


def test_emission_pool_validated():
