...
class Validator:
    def __init__(
        ...
        self.rancher = RancherClient(config)
    ...
    def run_cycle(self) -> None:
        ...
        try:
            ...
            log.warning("skip cycle: price feed unavailable: %s", exc)
        ...
        if orphans:
            log.info(
                "dropping %d unbound/spoofed labeled clusters", orphans
            )
        ...
        try:
            scores = compute_weights(
                ...
                hours_per_epoch,
                cfg.owner_uid,
                cfg.emission_usd_per_epoch(
                    tempo=tempo, hours=hours_per_epoch
                ),
            )
        ...
        owner_usd = max(
            0.0,
            cfg.emission_usd_per_epoch(tempo=tempo, hours=hours_per_epoch)
            - total_earned,
        )
        ...
        for inp in inputs:
            if inp.ready:
                for klass, count in inp.gpu_breakdown or (
                    ...
                    fleet_gpus[str(klass)] = (
                        fleet_gpus.get(str(klass), 0) + int(count)
                    )
        if fleet_gpus:
            log.info(
    ...
    def _targon_card(self) -> tuple[dict[str, float], str, float]:
        ...
        if self.hippius is not None:
            self._publish_payout(per_card)
        effective, _ = self.targon.effective_card()
        return effective, pricing_source, 0.0

    def _publish_payout(self, per_card: dict[str, float]) -> None:
        if self.dry_run:
    ...
    def _miner_gpu_breakdown(
        self, hotkey: str, clusters, nodes_by_cluster, include_tdx_non_cc: bool = False
    ) -> dict[str, int]:
    ...
    def _miner_gpu_posture(
        self, hotkey: str, clusters, nodes_by_cluster, ready: bool, include_tdx_non_cc: bool = False
    ...
    def _set_weights_once_per_epoch(self, weights: dict[int, float]) -> None:
        ...
        if self._last_submitted_epoch >= current_epoch:
            log.info("epoch %d: already attempted this epoch; not submitting", current_epoch)
            return
        ...
        if self._last_set_block is not None and blocks_since < rate_limit:
            ...
            log.info(
                ...
                current_epoch, blocks_since, rate_limit, remaining, remaining * 12,
            )
            return
        ...
        try:
            ...
            log.warning("set_weights rejected for epoch %d (will retry): %s", current_epoch, exc)
    ...
    def run_forever(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                log.error("unexpected cycle error:\n%s", traceback.format_exc())
