...
def fetch_published_payout() -> dict | None:
    """Anonymous GET of payout.json — the owner's last-known price snapshot.

    Returns None on any failure (callers fall back further). Readers use this
    to survive a Taostats outage: the owner publishes tao_usd + alpha_to_tao
    + per_card_usd every cycle, so a reader can keep scoring off the
    last-known prices for one cycle instead of skipping indefinitely.
    """
    try:
        return _get_public(DEFAULT_PAYOUT_KEY)
    except HippiusError:
        return None
...
class HippiusStore:
    ...
    def put_raw(
        ...
        self._put(
            key, data.encode() if isinstance(data, str) else data, content_type
        )
